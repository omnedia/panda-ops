import {ReviewResult, VCSAdapter} from '../config.js';
import {log} from './logger.js';

function summarizeResult(result: ReviewResult) {
    ...
    const hasIssues = errors + warns > 0;
...
function formatSummary(result: ReviewResult): string {
    ...
    return [
        ...
        `- ❌ Errors: ${stats.errors}`,
        `- ⚠️ Warnings: ${stats.warns}`,
        `- 💡 Tips: ${stats.tips}`,
        `- 📝 Notes: ${stats.notes}`,
        `- ✏️ Grammar: ${stats.grammar}`,
        "",
        `**Lines added:** ${stats.addedLines}`,
        `**AI Used:** ${stats.aiUsed ? "Yes" : "No"}`,
        "",
...
export function formatSummaryCLI(result: ReviewResult): string {
    const stats = summarizeResult(result);
...
export async function postReview(adapter: VCSAdapter, result: ReviewResult): Promise<void> {
