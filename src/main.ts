...
async function execute(cfg: PandaOpsConfig) {
  ...
  try {
    ...
    if (cfg.outputJson) {
      ...
      const summaryBlock = formatSummaryCLI(review, cfg);
      log.info('\n' + summaryBlock);
    } else {
      log.info('[PandaOps] Posting review comments...');
      await postReview(adapter, review, cfg);
...
async function main(argv: string[]) {
  ...
  program
    ...
    .option('--fail-on-warnings', 'Set REQUEST CHANGES on PR if warnings are found')
