...
export const ConfigSchema = z.object({
    ...
    openaiTemperature: z.number().default(0.2),
    openaiMaxTokens: z.number().default(800),
...
export function loadConfig(env: NodeJS.ProcessEnv, cli: Partial<PandaOpsConfig>): PandaOpsConfig {
    const merged = {
        ...
        openaiTemperature:
            cli.openaiTemperature ?? Number(env.OPENAI_TEMPERATURE ?? 0.2),
        openaiMaxTokens:
            cli.openaiMaxTokens ?? Number(env.OPENAI_MAX_TOKENS ?? 800),
        aiEnabled: cli.aiEnabled ?? (env.AI_ENABLED !== 'false'),
        maxComments: cli.maxComments ?? Number(env.MAX_COMMENTS ?? 50),

        focusErrors: cli.focusErrors ?? (env.AI_FOCUS_ERRORS !== 'false'),
        focusWarn: cli.focusWarn ?? (env.AI_FOCUS_WARN !== 'false'),
        focusTips: cli.focusTips ?? (env.AI_FOCUS_TIPS !== 'false'),
        focusNotes: cli.focusNotes ?? (env.AI_FOCUS_NOTES === 'true'),
        focusGrammar: cli.focusGrammar ?? (env.AI_FOCUS_GRAMMAR === 'true'),
    };
